...
RCT_EXPORT_METHOD(launchCamera:(NSString *)options callback:(RCTResponseSenderBlock)callback)
{
  ...
  [self launchImagePicker:RNImagePickerTargetCamera options:dict];
}
//RCT_REMAP_METHOD(launchImageLibrary,options :(NSString*)options resolver: (RCTPromiseResolveBlock)resolve
...
//  self.reject = reject;
//  [self launchImagePicker:RNImagePickerTargetLibrarySingleImage options:dict];
//}
//RCT_EXPORT_METHOD(launchImageLibrary:(NSString *)options callback:(RCTResponseSenderBlock)callback)
//{
//  NSData *data = [options dataUsingEncoding:NSUTF8StringEncoding];
//  NSDictionary *dict = (NSDictionary*)[NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
//  self.callback = callback;
...
RCT_REMAP_METHOD(getAImage, options:(NSString *)options resolver: (RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject)

{
 ...
 // self.callback = callback; // Save the callback so we can use it from the delegate methods
  self.resolve = resolve;
  self.reject = reject;
 self.options = [NSMutableDictionary dictionaryWithDictionary:dict]; // Set default options
  for (NSString *key in dict.keyEnumerator) { // Replace default options
    [self.options setValue:dict[key] forKey:key];
  }
  ...
  if ([UIAlertController class] && [UIAlertAction class]) { // iOS 8+
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:cancelTitle style:UIAlertActionStyleCancel handler:^(UIAlertAction * action) {
      NSDictionary *dict =@{@"didCancel": @YES};
      NSString *json = [self dictionnaryToJson:dict];
      self.resolve(@[json]); // Return callback for 'cancel' action (if is required)
    }];
...
- (void)actionHandler:(UIAlertAction *)action
{
  ...
  if (customButtonStr) {
    self.resolve(@[@{@"customButton": customButtonStr}]);
    return;
  }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
  dispatch_block_t dismissCompletionBlock = ^{
      ...
      if ([storageOptions objectForKey:@"path"]) {
        ...
        if (error) {
          NSLog(@"Error creating documents subdirectory: %@", error);
                     //self.callback(@[@{@"error": error.localizedFailureReason}]); quyRepare
          return;
        }
        else {
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
      ...
      if (imageURL && [[imageURL absoluteString] rangeOfString:@"ext=GIF"].location != NSNotFound) {
        ...
        [assetsLibrary assetForURL:imageURL resultBlock:^(ALAsset *asset) {
          ...
          NSString *json = [self dictionnaryToJson:response];
          self.resolve(@[json]);
         // self.callback(@[json]); quy repare
        } failureBlock:^(NSError *error) {
          self.reject(@"Error creating documents subdirectory quy",@"error quy",error);
          //self.callback(@[@{@"error": error.localizedFailureReason}]); quy repaire
        }];
        return;
      }
      ...
      if (error) {
        self.reject(@"Error creating documents subdirectory quy",@"error quy",error);
        //self.callback(@[@{@"error": error.localizedFailureReason}]); quy repart
        return;
      }
    ...
    [dicImage setObject:[response objectForKey:@"uri"] forKey:@"uri"];
    NSString *jsonImage = [self dictionnaryToJson:dicImage];
    self.resolve(@[jsonImage]);
   // self.callback(@[jsonImage]); quy repare
  };
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController *)picker
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [picker dismissViewControllerAnimated:YES completion:^{
      ...
      NSDictionary *dictOutput = @{@"didCancel": @YES};
      NSString *jsonData = [self dictionnaryToJson:dictOutput];
      self.resolve(@[jsonData]);
        //   self.callback(@[jsonData]); quy repare
...
- (UIImage*)downscaleImageIfNecessary:(UIImage*)image maxWidth:(float)maxWidth maxHeight:(float)maxHeight
{
  ...
  if (newImage == nil) {
    ...
    NSError *error;
    self.reject(@"could not scale image" ,@"quy",error);
  }
